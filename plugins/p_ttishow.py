...
"""-----------------------------------------https://t.me/flixoraoffiacial--------------------------------------"""

@Client.on_message(filters.new_chat_members & filters.group)
async def save_group(bot, message):
    Flixora_check = [u.id for u in message.new_chat_members]
    if temp.ME in Flixora_check:
        if not await db.get_chat(message.chat.id):
            total=await bot.get_chat_members_count(message.chat.id)
            Flixora_botz = message.from_user.mention if message.from_user else "Anonymous" 
            await bot.send_message(LOG_CHANNEL, script.LOG_TEXT_G.format(message.chat.title, message.chat.id, total, Flixora_botz))       
...
async def list_users(bot, message):
    Flixora = await message.reply('Getting List Of Users')
...
async def list_chats(bot, message):
    Flixora = await message.reply('Getting List Of chats')
