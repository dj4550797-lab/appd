...
import logging
from database.ia_filterdb import Flixora_get_movies, Flixora_get_series
...
async def Flixora_list_movies(client, message):
    try:
        movies = await Flixora_get_movies()
        if not movies:
        ...
        logger.error(f"Error in Flixora_list_movies: {e}")
...
async def Flixora_list_series(client, message):
    try:
        series_data = await Flixora_get_series()
        if not series_data:
        ...
        logger.error(f"Error in Flixora_list_series: {e}")
        await message.reply("An Error Occurred ☹️", parse_mode=ParseMode.HTML)