...
async def broadcast_users(bot, message):
    ...
    try:
        Flixora_user_response = await bot.listen(chat_id=message.chat.id, user_id=message.from_user.id, timeout=60)
    except asyncio.TimeoutError:
        await ask.delete()
        return await message.reply("❌ Timed out. Broadcast cancelled.")
    await ask.delete()
    if Flixora_user_response.text not in ("Yes", "No"):
        return await message.reply("❌ Invalid input. Broadcast cancelled.")

    is_pin = Flixora_user_response.text == "Yes"
    b_msg = message.reply_to_message
    users = [user async for user in await db.get_all_users()]
    total_users = len(users)
    Flixora_status_msg = await message.reply_text("📤 <b>Broadcasting your message...</b>")
    ...
    await Flixora_status_msg.edit(final_status)
...
async def broadcast_group(bot, message):
    ...
    Flixora_status_msg = await message.reply_text("📤 <b>Broadcasting your message to groups...</b>")
    start_time = time.time()
    ...
    try:
        await Flixora_status_msg.edit(Flixora_text)
    except MessageTooLong:
        with open("reason.txt", "w+") as outfile:
            outfile.write(str(failed))
        await message.reply_document(
            "reason.txt", caption=Flixora_text
        )
